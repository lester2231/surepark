...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(ref(db, "slots"), (snapshot) => {
      ...
      setSlots(formatted)
      // Sync selected slot data if modal is open
      if (selectedSlot) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] p-4 md:p-8 text-white font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight">SurePark Baguio</h1>
              <p className="text-slate-500 text-xs font-medium">Welcome, {user.email.split('@')[0]}</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button className="flex items-center gap-2 px-4 py-2 bg-[#10b981] hover:bg-[#059669] text-white rounded-lg text-sm font-bold transition-all shadow-md">
              <ScanLine size={18}/> Scanner
            </button>
            <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 bg-[#334155] hover:bg-[#475569] text-white rounded-lg text-sm font-bold transition-all">
              <RefreshCw size={18}/> Reset
            </button>
            <button onClick={() => { localStorage.removeItem("surepark_user"); router.push("/login") }} className="flex items-center gap-2 px-4 py-2 bg-[#ef4444] hover:bg-[#dc2626] text-white rounded-lg text-sm font-bold transition-all shadow-md">
              <LogOut size={18}/> Logout
            </button>
          </div>
        </div>

        {/* How to Use Section with Larger Icons */}
            ...
            <div className="px-6 pb-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {[
                  ...
                  { id: 2, title: "Reserve the Slot", desc: "Click View. You have 15 minutes to pay.", icon: <CalendarCheck size={24}/> },
                  ...
                  <div key={step.id} className="bg-[#1f2937]/50 p-5 rounded-lg border border-slate-700/50 flex gap-4 transition-all hover:border-blue-500/30">
                    <div className="w-12 h-12 rounded-xl bg-blue-600/10 flex items-center justify-center text-blue-500 shrink-0 shadow-inner">
                      {step.icon}
                    </div>
                    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
           ...
           <div className="bg-[#064e3b]/20 border border-green-500/20 p-4 rounded-xl flex items-center justify-between">
              <div><p className="text-[10px] font-bold text-green-400 uppercase tracking-widest">Available</p><p className="text-2xl font-bold">{stats.available}</p></div>
              <CheckCircle2 size={24} className="text-green-500 opacity-50"/>
           </div>
           <div className="bg-[#451a03]/20 border border-yellow-500/20 p-4 rounded-xl flex items-center justify-between">
              <div><p className="text-[10px] font-bold text-yellow-400 uppercase tracking-widest">Reserved</p><p className="text-2xl font-bold">{stats.reserved}</p></div>
              <Clock size={24} className="text-yellow-500 opacity-50"/>
           </div>
           <div className="bg-[#450a0a]/20 border border-red-500/20 p-4 rounded-xl flex items-center justify-between">
              <div><p className="text-[10px] font-bold text-red-400 uppercase tracking-widest">Occupied</p><p className="text-2xl font-bold">{stats.occupied}</p></div>
              <Car size={24} className="text-red-500 opacity-50"/>
           </div>
        </div>
          ...
          {filteredSlots.map((slot) => (
            <div key={slot.id} className="bg-[#111827]/60 border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-all shadow-xl">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-lg font-bold tracking-tight">{slot.name}</h3>
              ...
              <div className="text-2xl font-black mb-6">₱{slot.price} <span className="text-xs text-slate-600 font-bold italic">/hour</span></div>
              <button onClick={() => setSelectedSlot(slot)} className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-black tracking-widest uppercase transition-all shadow-lg">View Details</button>
            </div>
          ))}
        </div>
        ...
        {/* Detail Modal with Reserve Logic */}
        {selectedSlot && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <div className="bg-[#1e293b] border border-slate-700 rounded-3xl w-full max-w-md p-8 shadow-2xl">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setSelectedSlot(null)} className="p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-500"><XCircle size={32}/></button>
              </div>

              {selectedSlot.status === "available" ? (
                <div className="space-y-6">
                  <div className="bg-blue-500/5 border border-blue-500/20 p-6 rounded-2xl">
                    <p className="text-slate-400 text-xs font-bold uppercase mb-2 tracking-widest">Current Rate</p>
                    <p className="text-3xl font-black text-white">₱{selectedSlot.price}.00 <span className="text-sm font-medium italic text-slate-500">/ hour</span></p>
                  </div>
                  <button onClick={() => handleReserve(selectedSlot)} className="w-full py-4 bg-yellow-600 hover:bg-yellow-500 text-white rounded-xl font-black uppercase tracking-widest transition-all shadow-xl">Reserve Slot Now</button>
                  <p className="text-center text-[10px] text-slate-500 font-bold uppercase tracking-tighter italic">Note: You have 15 minutes to complete payment after reserving.</p>
                </div>
              ) : (
                <div className="bg-[#0f172a] p-6 rounded-2xl border border-blue-500/20 space-y-5">
                   <div className="flex justify-between items-center px-2">
                      <p className="text-xs font-bold text-slate-400 flex items-center gap-2"><Zap size={14} className={selectedSlot.status === 'occupied' ? "text-slate-600" : "text-yellow-400"}/> Bollard Control</p>
                   ...
                   >
                     {selectedSlot.status === 'occupied' ? <><ShieldCheck size={20}/> Safety Locked</> : (selectedSlot.bollardUp ? <><ArrowDown size={20}/> Lower Bollard</> : <><ArrowUp size={20}/> Raise Bollard</>)}
                   </button>
                </div>
              )}
            </div>
