...
export default function DashboardPage() {
  ...
  const handleReset = async () => {
    const resetData: Record<string, any> = {} // Fix for build error
    const ids = [1, 2, 3, 4, 5];
    ids.forEach((id) => {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-4 md:p-8 text-white">
      <div className="max-w-7xl mx-auto">
        {/* Header - Restored from image_99445a.jpg */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowScanner(!showScanner)} className="flex items-center gap-2 px-4 py-2 bg-green-600 rounded-lg text-sm font-medium"><QrCode size={16}/> Scanner</button>
            <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 bg-slate-700 rounded-lg text-sm font-medium"><RefreshCw size={16}/> Reset</button>
            <button onClick={() => { localStorage.removeItem("surepark_user"); router.push("/login") }} className="flex items-center gap-2 px-4 py-2 bg-red-600 rounded-lg text-sm font-medium"><LogOut size={16}/> Logout</button>
          </div>
        </div>

        {/* Tips Section - Restored from image_9b7a91.png */}
        ...
        {/* Filter & Horizontal Stats - Restored from image_994726.jpg */}
        <div className="mb-6">
        ...
        {/* Slot Grid - Restored from image_9b7a59.png */}
          ...
          {filteredSlots.map((slot) => (
            <div key={slot.id} className={`bg-[#1e293b]/50 border rounded-2xl p-6 transition-all ${
              ...
              {slot.status === 'reserved' && (
                <div className="bg-[#0f172a] p-3 rounded-lg mb-4">
                  <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Reserved by</p>
                  <p className="text-xs font-medium">{slot.reservedBy}</p>
                  {slot.reservedAt && <p className="text-[10px] text-yellow-500 mt-1 font-bold">Time left: {getTimeRemaining(slot.reservedAt)}</p>}
                </div>
              )}
            ...
            <div className="bg-[#1e293b] border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
              ...
              <div className="space-y-4">
                ...
                {selectedSlot.reservedBy === user.email && !selectedSlot.paid && (
                  <button onClick={() => handlePayment(selectedSlot)} className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold shadow-lg transition-all">Pay ₱{selectedSlot.price} Now</button>
                )}

                {selectedSlot.paid && (
                  <div className="bg-[#0f172a] p-5 rounded-2xl border border-blue-500/20 space-y-4">
                    <div className="flex justify-between items-center">
                      <p className="text-xs font-bold text-slate-400 flex items-center gap-2"><Zap size={14} className="text-yellow-400"/> Bollard Control</p>
                    ...
                    <button onClick={() => handleBollardToggle(selectedSlot)} className={`w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${selectedSlot.bollardUp ? 'bg-green-600 hover:bg-green-500' : 'bg-red-600 hover:bg-red-500'}`}>
                      {selectedSlot.bollardUp ? <><ArrowDown size={18}/> Lower Bollard</> : <><ArrowUp size={18}/> Raise Bollard</>}
                    </button>
                    {selectedSlot.status === 'occupied' && (
                      <p className="text-[9px] text-center text-green-400 font-bold uppercase tracking-widest"><ShieldCheck className="inline mb-0.5" size={10}/> Vehicle Detected by Hardware</p>
                    )}
                  </div>
                )}
              </div>
