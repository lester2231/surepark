      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          ...
          <div className="flex gap-2">
            <button className="flex items-center gap-2 px-4 py-2 bg-[#10b981] hover:bg-[#059669] text-white rounded-lg text-sm font-bold transition-all"><ScanLine size={18}/> Scanner</button>
            <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 bg-[#334155] hover:bg-[#475569] text-white rounded-lg text-sm font-bold transition-all"><RefreshCw size={18}/> Reset</button>
            <button onClick={() => { localStorage.removeItem("surepark_user"); router.push("/login") }} className="flex items-center gap-2 px-4 py-2 bg-[#ef4444] hover:bg-[#dc2626] text-white rounded-lg text-sm font-bold transition-all shadow-md"><LogOut size={18}/> Logout</button>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {[
                  ...
                  <div key={step.id} className="bg-[#1f2937]/50 p-5 rounded-lg border border-slate-700/50 flex gap-4">
                    <div className="w-8 h-8 rounded-full bg-blue-600/10 flex items-center justify-center text-blue-400 font-bold text-xs shrink-0">{step.id}</div>
                    <div>
                      <h4 className="text-white text-xs font-bold mb-1">{step.title}</h4>
                      <p className="text-slate-400 text-[10px] leading-relaxed font-medium">{step.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-[#064e3b]/20 border border-green-500/20 p-6 rounded-xl flex items-center justify-between">
          ...
          <div className="bg-[#451a03]/20 border border-yellow-500/20 p-6 rounded-xl flex items-center justify-between">
          ...
          <div className="bg-[#450a0a]/20 border border-red-500/20 p-6 rounded-xl flex items-center justify-between">
        ...
        {/* Slot Grid with View and Reserve Buttons */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {filteredSlots.map((slot) => (
            <div key={slot.id} className="bg-[#111827]/60 border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-all shadow-xl">
              ...
              <div className="text-2xl font-black mb-6">₱{slot.price} <span className="text-xs text-slate-600 font-bold">/hour</span></div>
              
              <div className="flex gap-2">
                <button onClick={() => setSelectedSlot(slot)} className="flex-1 py-3 bg-[#334155] hover:bg-[#475569] text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">View</button>
                {slot.status === "available" && (
                  <button onClick={() => handleReserve(slot.id)} className="flex-[1.5] py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-900/20">Reserve</button>
                )}
              </div>
            </div>
          ))}
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-[#1e293b] border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setSelectedSlot(null)} className="p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-400"><XCircle size={24}/></button>
              </div>
              <div className="bg-[#0f172a] p-5 rounded-2xl border border-blue-500/20 space-y-4">
                 <div className="flex justify-between items-center px-2">
                    <p className="text-xs font-bold text-slate-400 flex items-center gap-2"><Zap size={14} className="text-yellow-400"/> Bollard Status</p>
                 ...
                 <button 
                   ...
                   className={`w-full py-4 rounded-xl font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg ${
